...
function emit(type, fields, _ref) {
  ...
  if (currentClient && typeof currentClient[type] === 'function') {
    ...
    (0, _utils.warn)('The analytics client you provided doesn\'t support ' + type + ' events. Make sure that the analytics.js script is loaded.');
  }
}
...
function createTracker() {
  ...
  if (!options.client) {
    (0, _utils.warn)('Could not find an analytics client. Provide a client to' + 'createTracker or make sure that the analytics.js script' + 'is loaded and executed before your application code.');
  }
...
function buildMapper(customOptions) {
  return customOptions.skipDefaultMapping ? _extends({}, customOptions.mapper) : _extends({}, _configuration.defaultMapper.mapper, customOptions.mapper);
}
